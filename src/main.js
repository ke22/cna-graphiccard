...
async function main() {
  ...
  async function loadAndRender() {
    const baseMeta = await loadMeta(mission); // manifest 的資料來源/更新時間
    // 資料來源：URL ?sheet= 可覆寫為 Google Sheet 公開 CSV 網址（loader 已加 cache-bust）
    const { nodes, sheetMeta } = await loadNodes(mission, params.get('sheet') || undefined);

    // 標題：URL ?title= > sheet 標題欄 > 任務名稱
    const title = params.get('title') || sheetMeta.title || mission;
