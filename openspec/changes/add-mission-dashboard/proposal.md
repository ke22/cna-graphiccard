...
- 選定分頁後，dashboard 以 `index.html?sheet=<該 gid 的 gviz CSV 網址>&title=<分頁名稱>` 開啟既有模板；模板載入路徑沿用現有 `?sheet=` 覆寫機制，不變更資料載入邏輯。
- 模板工具列可從同一試算表分頁清單直接切換 mission。
...
- `mission-dashboard`: 連結 Google 試算表、列出分頁、挑選後以正確網址開啟既有時間軸模板的啟動入口。
