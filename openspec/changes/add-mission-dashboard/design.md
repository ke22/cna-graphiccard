...
CNA 圖卡產生器為純前端靜態網站（HTML/CSS/JS，無框架、無建置、`python -m http.server` 服務）。模板 `index.html` 由 `?mission=` 或 `?sheet=` 驅動：`src/main.js` 支援 `?sheet=` 覆寫資料來源，`?title=` 作為分頁名稱 fallback；若資料表內有 `標題` 欄，卡片標題以資料表為準。資料以 gviz CSV（`gid` 為單一分頁）載入，`src/csv-loader.js` 內含 cache-bust 與 RFC4180 解析。
...
- 重用模板既有 `?sheet=`／`?title=` 機制；`?title=` 僅作分頁名稱 fallback，資料表 `標題` 欄優先。
