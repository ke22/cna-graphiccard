...
- A content text block (color: `#12141A`) below the date label, preserving paragraph breaks as `<br><br>` or separate `<p>` elements
...
#### Scenario: Multi-paragraph content renders with visual separation
- **WHEN** a node's content contains `\n\n` (double newline)
- **THEN** the rendered output shows a visible gap between the two paragraphs (implemented as separate `<p>` tags or `<br><br>`)
